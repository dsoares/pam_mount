...
#define CFG_TOGGLED(_val) ( (_val[0] == 'Y'                                \
	...
	typedef const char *(*dotconf_callback_t) (command_t *,
						   context_t *);
...
/* ------ dotconf_command_loop_until_error() - like continue_line but return on the first error - */
	const char *dotconf_command_loop_until_error(configfile_t *
						     configfile);
...
*/
	char *dotconf_read_arg(configfile_t * configfile,
			       signed char **line);
...
/* ------ dotconf_callback() - register a special callback -------------------------------------- */
	void dotconf_callback(configfile_t * configfile,
			      callback_types type, dotconf_callback_t);
...
/* ------ path and pre must be freed by the developer ( dotconf_wild_card_cleanup) -- */
	int dotconf_find_wild_card(char *filename, char *wildcard,
				   char **path, char **pre, char **ext);
...
/* ------ internal utility function that determins if a string matches the '?' criteria -- */
	int dotconf_question_mark_match(char *dir_name, char *pre,
					char *ext);
...
/* ------ a '?' in name and calls the Internal Include function on that filename -- */
	int dotconf_handle_question_mark(command_t * cmd, char *path,
					 char *pre, char *ext);
