...
static void copy_word(signed char **dest, signed char **src, int max,
		      char term)
{
	signed char *cp1 = *src;
...
int dotconf_warning(configfile_t * configfile, int type,
		    unsigned long errnum, const char *fmt, ...)
{
	...
	if (configfile->errorhandler != 0) {	/* an errorhandler is registered */
		...
		retval =
		    configfile->errorhandler(configfile, type, errnum,
					     msg);
...
void dotconf_register_options(configfile_t * configfile,
...
#define GROW_BY   10
	...
	if (configfile->config_options == NULL)
		configfile->config_options =
		    malloc(sizeof(void *) * (GROW_BY + 1));
	else {
		if (!(num % GROW_BY))
			configfile->config_options =
			    realloc(configfile->config_options,
		      ...
		      dotconf_callback_t callback)
{
	switch (type) {
	case ERROR_HANDLER:
		configfile->errorhandler =
		    (dotconf_errorhandler_t) callback;
		break;
				...
				const char *delimit)
{
	...
	if (configfile->size <= 0) {
		if (stat(configfile->filename, &finfo)) {
			dotconf_warning(configfile, DCLOG_EMERG,
					ERR_NOACCESS,
	...
	while (!dotconf_get_next_line(buffer, CFG_BUFSIZE, configfile)) {
		...
		offset +=
		    snprintf((here_doc + offset),
			     configfile->size - offset - 1, "%s", buffer);
	}
	if (here_string)
...
char *dotconf_read_arg(configfile_t * configfile, signed char **line)
{
	...
	*line = cp1;
	...
	return buf[0] ? dotconf_substitute_env(configfile,
					       strdup(buf)) : NULL;
}
				     ...
				     const char *command)
{
	...
	for (option = 0, mod = 0; configfile->config_options[mod] && !done;
	     mod++)
		for (i = 0; configfile->config_options[mod][i].name[0];
		     i++) {
			if (!configfile->
			    cmp_func(name,
				     configfile->config_options[mod][i].
				     name, CFG_MAX_OPTION)) {
				option =
	...
	if ((option && option->name[0] == 0)
		...
		option =
		    (configoption_t *) & configfile->config_options[mod -
								    1][i];
	}
	...
	while (1) {
		...
		for (option = 0; configfile->config_options[mod] && !done;
			...
			for (opt_idx = next_opt_idx;
			     configfile->config_options[mod][opt_idx].
			     name[0]; opt_idx++) {
				if (!configfile->
				    cmp_func(name,
					     configfile->
		...
		if (!option)
			option =
			    get_argname_fallback(configfile->
						 config_options[1]);
		...
		if (!context_error)
			error =
			    dotconf_invoke_command(configfile, &command);
		else {
			if (!error) {
...
const char *dotconf_command_loop_until_error(configfile_t * configfile)
{
	...
	while (!(dotconf_get_next_line(buffer, CFG_BUFSIZE, configfile))) {
		const char *error =
		    dotconf_handle_command(configfile, buffer);
		if (error)
			return error;
	}
	return NULL;
				 ...
				 char *ext)
{
	...
	if ((dh = opendir(path)) != NULL) {
		while ((dirptr = readdir(dh)) != NULL) {
			match_state =
			    dotconf_question_mark_match(dirptr->d_name,
							pre, ext);

			if (match_state >= 0) {
				name_len = strlen(dirptr->d_name);
				new_path_len =
				    strlen(path) + name_len + strlen(ext) +
				    1;
...
DOTCONF_CB(dotconf_cb_include)
{
	...
	included =
	    dotconf_create(filename, cmd->configfile->config_options[1],
			   cmd->configfile->context,
			   cmd->configfile->flags);
	if (included) {
		included->contextchecker =
		    (dotconf_contextchecker_t) cmd->configfile->
		    contextchecker;
		included->errorhandler =
...
DOTCONF_CB(dotconf_cb_includepath)
{
	...
	if (!env)
		snprintf(cmd->configfile->includepath, CFG_MAX_FILENAME,
			 "%s", cmd->data.str);
	return NULL;
}
