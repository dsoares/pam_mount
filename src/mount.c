			   ...
			   const unsigned int vol, char *const mntpt)
{
	...
	if (config->volume[vol].type == SMBMOUNT) {
		...
		strncat(match, optlist_value(config->volume[vol].options, "user"), PATH_MAX - strlen(match));
	...
	/* FIXME: ugly hack to support umount.crypt script.  I hope that
		...
		for (i = 0; config->volume[vol].volume && i < PATH_MAX; i++) {
			if (config->volume[vol].volume[i] == '/')
				escaped_vol[i]  = '_';
			else
				escaped_vol[i] = config->volume[vol].volume[i];
		}
	...
	while (mtab_record = getmntent(mtab))
		...
		if (!strcasecmp(mtab_record->mnt_fsname, match)) {
 ...
 */
{
	...
	for (i = 0; config->command[i][UMOUNT]; i++)
		add_to_argv(_argv, &_argc, config->command[i][UMOUNT], vinfo);
	/* FIXME: ugly hack to support umount.crypt script.  I hope that
         * util-linux will have native dm_crypt support some day */
	     ...
	     const unsigned int vol, const char *password, const int mkmntpoint)
{
	...
	/* avoid bomb on mount command exiting before password written */
